...
#define set_En LATD |= (1 << 5)
#define clear_En LATD &= ~(1 << 5)

#define set_RS LATD |= (1 << 4)
#define clear_RS LATD &= ~(1 << 4)

#define LCD_set_MSB(val_) LATD = (LATD & 0xF0) | ((val_ & 0xF0) >> 4)
#define LCD_set_LSB(val_) LATD = (LATD & 0xF0) | ((val_ & 0x0F) >> 0)
...
#define ss_low LATA &= ~(1 << 5)
#define ss_high LATA |= (1 << 5)

void SPI_EEPROM_Init()
{
    // Configure ports as output and input
    TRISC &= ~(1 << 3); // SCK as output
    TRISC |= (1 << 4);  // SDI as input
    TRISC &= ~(1 << 5); // SDO as output
    TRISA &= ~(1 << 5); // CS as output

    ss_high;

    // input data sampled at middle of data output time
    SSPSTAT &= ~(1 << 7); // clearing the SMP bit

    // Clock polarity - CKP - SCK should be low when idle
    SSPCON1 &= ~(1 << 4); // CKP =0

    // SPI clock edge select should be done - risign edge - since ckp=0 and rising edge
    SSPSTAT |= (1 << 6); // CKE should be 1.

    // enable Serial ports
    SSPCON1 |= (1 << 5);

    // serial port mode as master and clcok - fosc/4
    SSPCON1 = (SSPCON1 & 0xF0) | (0x00);
}
...
void EEPROM_write_byte(int address_, uint8_t data_)
{
    ss_low;
...
unsigned char EEPROM_read(uint16_t address_)
{
    unsigned char data_;
    ss_low;
...
void main()
{
    ...
    EEPROM_write_string(0x0000, (unsigned char *)"hi hello and welcome");
    __delay_ms(10);
