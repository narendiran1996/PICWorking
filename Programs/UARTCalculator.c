...
void UART_TX_sendInt(int NumVal)
{
    char buff[16];
    itoa(NumVal, buff, 10);
    UART_TX_sendString(buff);
}
void UART_RX_Init()
{
    SPBRG = 25;        // baudrate calculated based for BRGH = 1  using SPBRG = ((FOSC/baud)/16) - 1
...
char ptrToString[20];
void UART_RX_receiveStringUntil7()
{
...
int noVal = 0;
void UART_RX_receiveStringUntilG()
{
    noVal = 0;
    unsigned char currentChar = UART_RX_receiveChar();
    while (currentChar != 'G')
    {
        noVal = (noVal * 10) + (currentChar - 48);
        currentChar = UART_RX_receiveChar();
    }
}
...
void sendCalculator()
{
    ...
    UART_TX_sendString("Enter First Number: ");
    UART_RX_receiveStringUntilG();
    // LCD_set_cursor(1, 1);
    // LCD_print_int(noVal, 10);
    int no1 = noVal;
    UART_TX_sendInt(no1);
    UART_TX_sendString("\n\r");
    UART_TX_sendString("Enter Second Number: ");
    UART_RX_receiveStringUntilG();
    int no2 = noVal;
    UART_TX_sendInt(no2);

    UART_TX_sendString("\n\r");
    UART_TX_sendString("Enter Operation: ");
    unsigned char oper = UART_RX_receiveChar();
    UART_TX_sendChar(oper);
    ...
    UART_TX_sendString("\n\r");
    UART_TX_sendString("Result = ");
    UART_TX_sendInt(results);
    UART_TX_sendString("\n\r");
}
